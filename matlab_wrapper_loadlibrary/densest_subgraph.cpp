...
{ 
    ...
    size_t outputlen = 0;
    double density = densest_subgraph(n, m, ei, ej, w, output, &outputlen);
    plhs[0] = mxCreateDoubleMatrix(1,1,mxREAL);
    plhs[1] = mxCreateDoubleMatrix(1,1,mxREAL);
    plhs[2] = mxCreateDoubleMatrix(outputlen,1,mxREAL);
    *(mxGetPr(plhs[0])) = density;
    *(mxGetPr(plhs[1])) = outputlen;
    for(int i = 0; i < outputlen; i ++)
    {
	*(mxGetPr(plhs[2]) + i) = output[i];
    }
} 
